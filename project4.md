...
begin
            ...
            case current_state is
                when PLAYING =>
                    if dir = '0' then -- 向右
                        ...
                        elsif ball_reg = "00000001" then -- 右邊漏接
                            score_l <= score_l + 1;
                        ...
                        elsif ball_reg = "10000000" then -- 左邊漏接
                            score_r <= score_r + 1;
                            current_state <= SHOW_SCORE;
                        else
                ...
                when SHOW_SCORE =>
                    -- 分數顯示停留約 10 個 slow_clk 週期
                    if delay_cnt < 3 then
                        delay_cnt <= delay_cnt + 1;
                    else
                        delay_cnt <= 0;
                        ball_reg <= "10000000"; -- 重回左發球
                        dir <= '0';
                        current_state <= PLAYING;
                    end if;
            end case;
    ...
    -- 3. LED 輸出邏輯：根據狀態切換顯示內容
    -- PLAYING 時顯示球, SHOW_SCORE 時顯示 [左比分(4-bit) | 右比分(4-bit)]
