...
architecture Behavioral of pingpong_top is
    ...
    signal var_div_cnt : unsigned(26 downto 0) := (others => '0');
...
begin
    ...
    process(clk, rst)
        variable target : unsigned(26 downto 0);
    begin
        if rst = '1' then
            var_div_cnt <= (others => '0');
            trigger <= '0';
        elsif rising_edge(clk) then
            trigger <= '0';

            target := "01" & unsigned(lfsr_reg) & "000000000000000000000"; 
            
            if var_div_cnt >= target then
                var_div_cnt <= (others => '0');
                trigger <= '1'; 
            else
                var_div_cnt <= var_div_cnt + 1;
            end if;
        end if;
    end process;
